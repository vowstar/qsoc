...
/* The value one slow channel carries on one role, as the harness sees it. */
QString lsRoleExpression(const QSocIomuxLsChannelPlan &channel, QSocIomuxRole role)
{
    const QSocIomuxEndpointPlan &endpoint
        = roleOf(const_cast<QSocIomuxLsChannelPlan &>(channel), role);
    if (!endpoint.link.isEmpty()) {
        const QString name = QSocIomuxGenerator::lsPortName(channel.channel, role);
    ...
    const QSocIomuxPlan &plan, quint32 pin, quint32 slot, QSocIomuxRole role)
{
    ...
    if (slot == 0 && pool >= 0) {
        /* Slot 0 of a pool member follows its channel select over the pool's
         * channels and is zero for any other number. */
        QString expression;
...
QString buildSystemVerilog(const QSocIomuxPlan &plan)
{
        ...
        for (const QSocIomuxLsChannelPlan &channel : pool.channels) {
            for (const QSocIomuxRole role :
                 {QSocIomuxRole::InputValue,
                  ...
                  QSocIomuxRole::OutputValue,
                  QSocIomuxRole::OutputEnable}) {
                if (roleOf(const_cast<QSocIomuxLsChannelPlan &>(channel), role).link.isEmpty()) {
                    continue;
                }
                const QString name = QSocIomuxGenerator::lsPortName(channel.channel, role);
                if (role == QSocIomuxRole::InputValue) {
    ...
    for (quint32 pin = 0; pin < plan.pinCount; ++pin) {
        ...
        for (const QSocIomuxCorePort &port :
             QSocIomuxGenerator::coreLsPinPorts(plan, pin)
                 + QSocIomuxGenerator::corePinOptionPorts(plan, pin)) {
            declarations.append(
                port.width == 1
    ...
    if (plan.hasLs()) {
        lines.append(QString("logic [%1:0] ls_tx_input_enable_w;").arg(lsChannels - 1));
        lines.append(QString("logic [%1:0] ls_tx_output_value_w;").arg(lsChannels - 1));
        lines.append(QString("logic [%1:0] ls_tx_output_enable_w;").arg(lsChannels - 1));
        lines.append(QString("logic [%1:0] ls_rx_input_value_w;").arg(lsChannels - 1));
    }
    ...
    if (plan.hasLs()) {
        ...
        for (const QString &name : txPorts + rxPorts) {
            if (name.startsWith("ls_c")) {
                connConnections.append(QString("    .%1(%1)").arg(name));
            }
        }
    }
    ...
    for (quint32 pin = 0; pin < plan.pinCount; ++pin) {
        ...
        /* A pool's slow inputs are proven in the block of its first pin, so
         * one bank owns them. */
        const int pool = plan.lsPoolOf(pin);
        if (pool >= 0 && plan.lsPools.at(pool).pins.first() == pin) {
            for (const QSocIomuxLsChannelPlan &channel : plan.lsPools.at(pool).channels) {
                if (!channel.hasSink()) {
                    continue;
                }
                ...
                if (plan.option.rxOverride) {
                    expression = QString("ls_c%1_rx_src_i ? ls_c%1_rx_value_i : (%2)")
                                     .arg(channel.channel)
                                     .arg(expression);
                }
                if (plan.option.invert) {
                    expression
                        = QString("(%1) ^ ls_c%2_rx_inv_i").arg(expression).arg(channel.channel);
                }
                if (channel.inputValue.invert) {
                    expression += QStringLiteral(" ^ 1'b1");
                }
                lines.append(
                    QString("    assert (%1 == (%2));")
                        .arg(
                            QSocIomuxGenerator::lsPortName(channel.channel, QSocIomuxRole::InputValue),
                            expression));
            }
        }
        lines.append("end");
    ...
    lines.append("always_comb begin");
    for (quint32 slot = 0; slot < plan.hsSlots; ++slot) {
        /* Named, because an unnamed cover arrives at a front end as one more
         * copy of the same anonymous label and comes back renamed. */
        lines.append(QString("    cover_pin_0_slot_%1: cover (pin_0_select_i == %2'd%1);")
                         .arg(slot)
                         .arg(width));
        if (plan.pinCount > 1) {
            lines.append(QString("    cover_pin_%1_slot_%2: cover (pin_%1_select_i == %3'd%2);")
                             .arg(plan.pinCount - 1)
                             .arg(slot)
                             .arg(width));
        }
...
QSocIomuxFormalCollateral QSocIomuxFormal::generatePad(const QSocIomuxPlan &plan)
{
     ...
     * lends its name to two modules and is never instantiated under its own,
     * so stubbing the key as well would leave a module nothing refers to. */
    const auto sidesOf = [&](const QString &cell) {
        QStringList used;
        if (!plan.ioRing.declared || !plan.ioLib.contains(cell)
            || plan.ioLib.value(cell).axis.isEmpty()) {
            used.append(QString());
            return used;
        }
        for (const QString &side : QSocIomuxGenerator::ringSides()) {
            if (!plan.ioRing.side.value(side).isEmpty()) {
                used.append(side);
            }
        }
        return used;
    ...
    for (quint32 pin = 0; pin < plan.pinCount; ++pin) {
        ...
        if (cell.declared()) {
            stubs[plan.padModule(pin)] = {cell.cellPorts, &cell};
        }
    }
    for (const QSocIoRingDirect &direct : plan.ioRing.direct) {
        for (const QString &side : sidesOf(direct.cell)) {
            const QString module = plan.ringModule(direct.cell, side);
    ...
    if (plan.ioRing.declared) {
        ...
        for (const QString &base : physical) {
            for (const QString &side : sidesOf(base)) {
                const QString module = plan.ringModule(base, side);
                if (!stubs.contains(module)) {
