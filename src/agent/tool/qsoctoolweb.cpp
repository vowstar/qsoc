...
/* Pick the on-wire encoding format for the inlined image. PNG is kept
 * only when the source already has alpha (transparent UI captures); for
 * everything else JPEG quality 80 is roughly an order of magnitude
 * smaller than PNG with negligible model-relevant detail loss. */
...
/* Encode QImage to bytes; returns empty buffer if encoding fails. */
    ...
    const QString &sourceUrl, const QString &contentType, const QByteArray &body)
{
    ...
    if (!buf.open(QIODevice::ReadOnly)) {
        return QString("Error: failed to read image body (mime: %1)").arg(contentType);
    }
     ...
     * the model still knows the URL referenced an image and can decide
     * whether to ask the user for an alternative. */
    LLMModelConfig modelCfg;
    bool           accepts = false;
    if (llmService) {
        modelCfg = llmService->getCurrentModelConfig();
    ...
    /* Resize when the image either exceeds the model's token budget or
     * violates the configured maximum dimension. The same QImage decode
     * also lets us re-encode as JPEG to shrink wire bytes; bytes do not
     * affect token cost but they matter for the 1 MB request cap. */
    ...
    const char *encFormat = encodingFormatFor(decoded, contentType);
    ...
    nlohmann::json payload = {
        ...
        {"source_url", sourceUrl.toStdString()},
        {"width", finalW},
        {"height", finalH},
