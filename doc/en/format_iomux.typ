...
control with one row has nothing to select: its pins take that row and it
owns no field and no port.
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [`port`, route], [`input_value` `input_enable` `output_value` `output_enable`], [the four roles],
